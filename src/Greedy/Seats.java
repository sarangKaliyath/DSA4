...
without having any vacant seat between them in such a way that the total
...
public class Seats {
    public static void main(String[] args) {
        String A = "....x..xx...x..";
        int res = find(A, A.length(), (int) Math.pow(10, 7) + 3);
        System.out.println(res);
        // Time O(N);
        // Space O(N);
    }

    public static int find(String A, int n, int MOD) {
        ...
        int occupiedPos = pos.get(median) - 1;
        long count = 0;

        for (int i = median - 1; i >= 0; i--) {
            count = (count + occupiedPos - pos.get(i)) % MOD;
            occupiedPos--;
        }

        occupiedPos = pos.get(median) + 1;
        for (int i = median + 1; i < pos.size(); i++) {
            count = (count + pos.get(i) - occupiedPos) % MOD;
            occupiedPos++;
        }
